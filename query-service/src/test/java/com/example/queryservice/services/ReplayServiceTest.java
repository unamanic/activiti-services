...
class ReplayServiceTest {
    ...
    @Test
    void replay_returnsFalse_whenNoAuditEventsFound() throws Exception {
        when(auditEventRepository.findAll(any(Specification.class))).thenReturn(List.of());

        CompletableFuture<Boolean> result = replayService.replay("proc-1");
    ...
    void replay_returnsTrue_andPurgesAndReplays_whenEventsExist() throws Exception {
    ...
    @Test
    @SuppressWarnings({"unchecked", "rawtypes"})
    void replay_purgesBeforeReplaying() throws Exception {
        AuditEventEntity auditEvent = mock(AuditEventEntity.class);
        ...
        replayService.replay("proc-2").get();
